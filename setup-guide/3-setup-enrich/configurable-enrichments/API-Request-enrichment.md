...
* `jsonPath` lets you provide the [JSON Path statement][jsonpath] to navigate to the field inside the JSON that you want to use as the input.
...
This enrichment uses any 3rd party RESTful service to fetch data in JSON format.
In most cases however you probably want to use your own private server to maintain acceptable performance since third-party service may cause serious slowdown of your enrichment process.
